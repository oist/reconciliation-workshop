...
static double newton(corax_partition_t *   partition,
                     ...
                     double              initial_length)
{
  ...
  double *sumtable = (double *)corax_aligned_alloc(
  ...
  for (i = 0; i < MAX_ITER; ++i)
  {
    ...
    double opt_logl = corax_compute_likelihood_derivatives(partition,
                                                         parent_scaler_index,
                                                         child_scaler_index,
                                                         len,
                                                         params_indices,
...
int main(int argc, char *argv[])
{
  ...
  /* create the CORAX partition instance */
  partition = corax_partition_create(
      ...
      CORAX_ATTRIB_ARCH_CPU); /* do not use vectorizations */
  ...
  corax_compute_gamma_cats(alpha, RATES, rate_cats, CORAX_GAMMA_RATES_MEAN);
  ...
  /* set the 5 tip CLVs, and use the corax_map_nt map for converting
     the sequences to CLVs */
  corax_set_tip_states(partition, 0, corax_map_nt, "WAAAAB");
  corax_set_tip_states(partition, 1, corax_map_nt, "CACACD");
  corax_set_tip_states(partition, 2, corax_map_nt, "AGGACA");
  corax_set_tip_states(partition, 3, corax_map_nt, "CGTAGT");
  ...
  /* update five probability matrices using the rate matrix with
     ...
     (substitution rates + frequencies) params_indices[i], and can be refereed
  ...
  for (i = 0; i < 5; ++i)
  {
    ...
    corax_show_pmatrix(partition, i, 7);
    printf("\n");
  }

  /* create an operations array for specifying the traversal
     descriptor when computing the CLVs */
  operations = (corax_operation_t *)malloc(2 * sizeof(corax_operation_t));
  ...
  printf("Tip 0: ");
  corax_show_clv(partition, 0, CORAX_SCALE_BUFFER_NONE, 7);
  printf("Tip 1: ");
  corax_show_clv(partition, 1, CORAX_SCALE_BUFFER_NONE, 7);
  printf("Tip 2: ");
  corax_show_clv(partition, 2, CORAX_SCALE_BUFFER_NONE, 7);
  printf("Tip 3: ");
  corax_show_clv(partition, 3, CORAX_SCALE_BUFFER_NONE, 7);

  printf("CLV 4: ");
  corax_show_clv(partition, 4, 0, 7);
  printf("CLV 5: ");
  corax_show_clv(partition, 5, 1, 7);
  ...
  double logl = corax_compute_edge_loglikelihood(
