...
int main(int argc, char *argv[])
{
  ...
  if (argc != 5) fatal(" syntax: %s [fasta] [seed] [attrib] [states]", argv[0]);

  /* open FASTA file */
  pll_fasta_t *fp = pll_fasta_open(argv[1], pll_map_fasta);
  if (!fp) fatal("Error opening file %s", argv[1]);
  ...
  unsigned int *weight =
      pll_compress_site_patterns(seqdata, map, tip_nodes_count, &sites);
  ...
  */
  ...
  pll_utree_t *tree =
      pll_fastparsimony_stepwise(&parsimony, headers, &score, 1, atoi(argv[2]));
  printf("Score: %u\n", score);
