...
#define MODENAME(m) (m == CORAX_GAMMA_RATES_MEAN ? "MEAN" : "MEDIAN")

static double       alpha[N_ALPHAS]     = {0.1, 1.25, 100};
static unsigned int n_cat_gamma[N_CATS] = {1, 4, 6};

static int modes[N_MODES] = {CORAX_GAMMA_RATES_MEDIAN, CORAX_GAMMA_RATES_MEAN};
...
int main(int argc, char *argv[])
{
  ...
  if (corax_compute_gamma_cats(invalid_alpha, 4, rate_cats, CORAX_GAMMA_RATES_MEAN)
      == CORAX_FAILURE)
  {
  ...
  for (k = 0; k < N_CATS; ++k)
  {
    ...
    double frequencies[N_STATES]   = {0.12, 0.14, 0.13, 0.11, 0.15, 0.13, 0.12};
    unsigned int matrix_indices[4] = {0, 1, 2, 3};
    double subst_params[N_SUBST_PARAMS] = {0.5, 2,   3,   4,   5,   1.1, 1.2,
                                           1.3, 1.4, 1.5, 2.1, 2.2, 2.3, 2.4,
                                           2.5, 3.1, 3.2, 3.3, 3.4, 3.5, 1};
    ...
    int check_states = 1;
    check_states &=
        corax_set_tip_states(partition, 0, odd_map, "BAACDCGCDA--AEECFAAD");
    check_states &=
        corax_set_tip_states(partition, 1, odd_map, "CACCABGCBA--BDDGFCDA");
    check_states &=
        corax_set_tip_states(partition, 2, odd_map, "AG-C-CGCAG--CGFCFACC");
    check_states &=
        corax_set_tip_states(partition, 3, odd_map, "CGDCBDGCAA--AB-C-AAG");
    check_states &=
        corax_set_tip_states(partition, 4, odd_map, "CGACFFGCCA--AF-D-AAG");
    ...
    for (i = 0; i < N_ALPHAS; ++i)
    {
      for (m = 0; m < N_MODES; ++m)
      {
        ...
        if (corax_compute_gamma_cats(
                alpha[i], n_cat_gamma[k], rate_cats, modes[m])
            == CORAX_FAILURE)
        {
        ...
        printf("[%d] CLV 7: ", i);
        corax_show_clv(partition, 7, CORAX_SCALE_BUFFER_NONE, FLOAT_PRECISION);
