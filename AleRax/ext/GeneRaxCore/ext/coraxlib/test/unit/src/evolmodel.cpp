...
TEST(EvolModel, defaults)
{
  // buildup
  auto model = EvolModel();
...
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "GTR+F+G");
...
TEST(EvolModel, GTR_R_user)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "GTR{0.1/0.5/1/2/3.5/0.5}+R3+Fc");
  ...
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_SUBST_RATES), ParamMode::user);
...
TEST(EvolModel, JCI_user)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "JC+G+IU{0.7}");
...
TEST(EvolModel, JCFG_user)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "JC+FC+G{2.5}");
  ...
  EXPECT_EQ(model.ratehet_mode(), CORAX_UTIL_MIXTYPE_GAMMA);
  EXPECT_EQ(model.num_ratecats(), 4);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_FREQUENCIES), ParamMode::empirical);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_ALPHA), ParamMode::user);
  EXPECT_EQ(model.params_to_optimize(), 0);
  EXPECT_EQ(model.alpha(), 2.5);
  EXPECT_EQ(model.num_free_params(), 3);
}

TEST(EvolModel, HKY_user_freqs)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "HKY+FU{0.1/0.299/0.201/0.4}");
  ...
  EXPECT_EQ(model.ratehet_mode(), CORAX_UTIL_MIXTYPE_FIXED);
  EXPECT_EQ(model.num_ratecats(), 1);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_FREQUENCIES), ParamMode::user);
...
TEST(EvolModel, HKY_user_rates)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "HKY{1/2.424}+FC");
...
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "LG+F+I+G8a");
...
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "LG4X");
...
TEST(EvolModel, LG_R4_user)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "LG+R4{1/2/3/4}");
  ...
  EXPECT_EQ(model.num_ratecats(), 4);
  EXPECT_EQ(model.params_to_optimize(), 0);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_FREE_RATES), ParamMode::user);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_RATE_WEIGHTS), ParamMode::user);
...
TEST(EvolModel, aliases)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "TrNef+G");
  ...
  model = EvolModel(DataType::autodetect, "TrN");
  EXPECT_EQ(model.name(), "TN93");
  EXPECT_EQ(model.num_free_params(), 5);
  model = EvolModel(DataType::autodetect, "TPM1+FE");
  EXPECT_EQ(model.name(), "K81");
  EXPECT_EQ(model.num_free_params(), 2);
  model = EvolModel(DataType::autodetect, "TPM1uf");
  EXPECT_EQ(model.name(), "K81uf");
  EXPECT_EQ(model.num_free_params(), 5);
}

TEST(EvolModel, multistate)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "MULTI8_MK+G");
  ...
  model = EvolModel(DataType::autodetect, "MULTI53_GTR");
  EXPECT_EQ(DataType::multistate, model.data_type());
...
TEST(EvolModel, multistate_custom)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "MULTI6_GTR+M{AbCDef}{X-?}");
  ...
  model = EvolModel(DataType::autodetect, "MULTI5_MK+Mi{AbCdF}+F+G+I");
...
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "DNA001122");
  ...
  EXPECT_EQ(model.ratehet_mode(), CORAX_UTIL_MIXTYPE_FIXED);
  EXPECT_EQ(model.num_ratecats(), 1);
  EXPECT_EQ(model.param_mode(CORAX_OPT_PARAM_SUBST_RATES), ParamMode::ML);
  EXPECT_EQ(model.params_to_optimize(), CORAX_OPT_PARAM_SUBST_RATES | CORAX_OPT_PARAM_FREQUENCIES);
  EXPECT_EQ(model.num_free_params(), 5);
}

TEST(EvolModel, multistate_usersym)
{
  // buildup
  auto model = EvolModel(DataType::autodetect, "MULTI5_USER1234561231+FC+G");
